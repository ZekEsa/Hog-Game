...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    sum, current_roll, sow_sad = 0, 0, False
    while (num_rolls > 0):
        current_roll = dice()
...
def tail_points(opponent_score):
    ...
    if(opponent_score >= 10):
        ...
        return 2 * abs(opponent_score%10 - ones) + 1
...
def perfect_square(num):
    square = sqrt(num)
    if ( int(square) ** 2 != num ): return False
    return True
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def strategy_always(player_score, oppenent_score):
        return n
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    if ( tail_points(opponent_score) >= threshold ): return 0
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    #temp_score = score + tail_points(opponent_score) # score if pig tail rule
    temp_score = square_update(0, score, opponent_score) # score if Square Swine

    if( (temp_score - score) >= threshold ): return 0
    return num_rolls  # Remove this line once implemented.
